#!/usr/bin/env python3
"""Test the restraint hypothesis: warmth-independence = restraint semantics.

From comprehensive analysis of 45 terms: every term independent on 2+ models
(careful, thorough, deliberate, cautious, methodical, patient, prudent,
measured, rigorous) describes restraint/discipline/self-control.

HYPOTHESIS: warmth-independence is predicted by RESTRAINT semantics, not
distributional context. Terms about holding-back/control are independent;
terms about positive quality/outcomes are biased.

PREDICTIONS:
  Independent (restraint/discipline):
    restrained/unrestrained — holding back, self-control
    disciplined/undisciplined — following rules, self-control
    temperate/intemperate — moderation, not excessive

  Biased (positive valence/quality outcome):
    exceptional/mediocre — positive quality evaluation
    remarkable/unremarkable — positive quality evaluation
    brilliant/dull — positive cognitive/quality evaluation

Null "all biased" scores 3/6.
If hypothesis correct: 6/6 (or close).
"""
...
PREDICTED_BIASED = [
    ("exceptional", "Exceptional", "Mediocre"),
    ("remarkable", "Remarkable", "Unremarkable"),
    ("brilliant", "Brilliant", "Dull"),
]

ALL_TEST = PREDICTED_INDEPENDENT + PREDICTED_BIASED

REFERENCE = [("good", "Good", "Bad"), ("careful", "Careful", "Reckless")]
...
def main():
    ...
    for model_name in MODELS:
        ...
        for name, pos, neg in REFERENCE + ALL_TEST:
        ...
        print(f"\n{'Term':15s} {'Acc':>5s} {'Orig':>5s} {'Warm':>5s} {'r_good':>7s} {'Prediction':>12s} {'Correct?':>8s}")
        print("-" * 70)

        model_results = {}
        for name, pos, neg in ALL_TEST:
            ...
            print(f"{name:15s} {acc:5.0%} {orig_acc:5.0%} {warm_acc:5.0%} {r:+7.2f} {predicted:>12s} {mark:>8s}")
        ...
        print(f"\nPrediction accuracy: {n_correct}/6")

        careful_acc = sum(axis_correct["careful"]) / n
        r_careful, _ = pearsonr(axis_deltas["careful"], good_deltas)
        print(f"Reference: careful acc={careful_acc:.0%}, r_good={r_careful:+.2f}")
    ...
    total_correct = sum(
        1 for name, _, _ in ALL_TEST
        if sum(1 for m in all_results if all_results[m][name]["prediction_correct"]) >= 2
    )
    null_score = len(PREDICTED_BIASED)
    print(f"\nRestraint hypothesis: {total_correct}/6 correct on >= 2/3 models")
    print(f"Null 'all biased': {null_score}/6")
    print(f"Discrimination: {total_correct - null_score:+d} above null")

    out = ROOT / "notes/research_cycles/tree_decomposition/restraint_hypothesis_test.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps({
        "experiment": "Restraint hypothesis test",
        "date": "2026-06-28",
        "hypothesis": "Warmth-independence = restraint/discipline semantics",
        "design": {
            "n_independent_predictions": len(PREDICTED_INDEPENDENT),
            "n_biased_predictions": len(PREDICTED_BIASED),
