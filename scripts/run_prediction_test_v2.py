#!/usr/bin/env python3
"""Second pre-registered prediction test — designed to discriminate from null.

CONTEXT: The first prediction test (§4.21) scored 8/10 but the trivial null
"all evaluative terms are warmth-biased" also scores 8/10 (because only 2/10
terms were actually independent). This test inverts the ratio: 6 predicted
independent, 4 predicted biased. The null now scores only 4/10.

POST-HOC HYPOTHESIS BEING TESTED (generated by first prediction test):
  Warmth-independence requires the term to appear primarily in task/quality
  contexts, not personality descriptions.

PREDICTIONS (declared before running):
  Warmth-INDEPENDENT (task/quality register, predicted |r_good| < 0.3 on 2/3+ models):
    systematic/disorganized — "systematic review," "systematic approach" (rarely about people)
    rigorous/lax — "rigorous analysis," "rigorous testing" (rarely personality)
    stringent/lenient — "stringent requirements" (never about people)
    accurate/inaccurate — "accurate description" (quality of output, not person)
    logical/illogical — "logical argument" (cognitive, not social)
    analytical/muddled — "analytical thinking" (cognitive, not warmth)

  Warmth-BIASED (personality-praise register, predicted r_good > 0.4 on 2/3+ models):
    admirable/contemptible — "admirable person" (direct praise)
    noble/ignoble — "noble character" (virtue/warmth)
    generous/selfish — "generous person" (directly warm)
    worthy/unworthy — "worthy person" (praise/value judgment)

SUCCESS CRITERION: Score significantly above 4/10 (the null's score).
If >= 8/10, hypothesis has genuine predictive power.
"""
...
def main():
    ...
    for model_name in MODELS:
        ...
        for name, pos, neg in ALL_TEST_AXES:
            ...
            print(f"{name:15s} {acc:5.0%} {orig_acc:5.0%} {warm_acc:5.0%} {r:+7.2f} {p_val:7.3f} {predicted:>12s} {mark:>8s}")
    ...
    print(f"\nOur predictions: {total_correct}/10 correct on >= 2/3 models")
    print(f"Null 'all biased': {null_score}/10")
    print(f"Discrimination: {total_correct - null_score:+d} above null")

    out = ROOT / "notes/research_cycles/tree_decomposition/prediction_test_v2.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps({
        "experiment": "Second prediction test — discriminating from null",
        "date": "2026-06-28",
        "design": {
            "hypothesis": "Warmth-independence requires task/quality distributional context, not personality-praise",
            "n_independent_predictions": len(PREDICTED_INDEPENDENT),
            "n_biased_predictions": len(PREDICTED_BIASED),
            "null_score": null_score,
            "success_criterion": "Score significantly above 4/10 (null)"
        },
        "predictions": {
            "independent": [x[0] for x in PREDICTED_INDEPENDENT],
            "biased": [x[0] for x in PREDICTED_BIASED],
            "criterion_independent": "|r_good| < 0.3 on >= 2/3 models",
            "criterion_biased": "r_good > 0.4 on >= 2/3 models",
        },
        "results": all_results,
